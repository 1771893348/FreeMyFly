...
package com.wgw.freemyfly.imgload;

/**
 * Created by wgw on 2018/5/30.
 */
...
public class GlideUtils {
    ...
    public static void loadImageCrop(Context mContext, String path, ImageView mImageView) {
        Glide.with(mContext).load(path).apply(RequestOptions.centerCropTransform().centerCrop().diskCacheStrategy(DiskCacheStrategy.ALL)).into(mImageView);
    }
    ...
    public static void loadImageViewPriority(Context mContext, String path, ImageView mImageView) {
        Glide.with(mContext).load(path).apply(RequestOptions.priorityOf(Priority.NORMAL).centerCrop()).into(mImageView);
